...
#define PLUGIN_VERSION				"BETA 1.6"
...
#define SENTRYBUSTER_DISTANCE		380.0 //original value 410.0 changed because particle wasn't matching up
...
public Action:OnPlayerRunCmd( iClient, &iButtons, &iImpulse, Float:flVelocity[3], Float:flAngles[3], &iWeapon )
{
	...
	new TFClassType:iClass = TF2_GetPlayerClass(iClient);
	if( bInRespawn[iClient] && iClass == TFClass_Scout )
	{
		//iButtons &= ~IN_ATTACK;
		iButtons &= ~IN_ATTACK2;
...
public OnPostInventoryApplication( Handle:hEvent, const String:strEventName[], bool:bDontBroadcast )
{
	...
	switch( iClass )
	{
		...
		{	
			...
			if( iRobotVariant[iClient] == 4 ) //wrap assasin old Tornado Scout
			{
		...
		case TFClass_Medic:
		{
			...
			if( iRobotVariant[iClient] >= 3 && iRobotVariant[iClient] <= 6 )
			{
				...
				else if( iRobotVariant[iClient] == 6 )//old Giant Medic		//Benoist3012: "Wow you have found my message x), the giant medic will be back in cfg variant!!!" // its actually back!
				{
					...
					KillVaccinatorBackpack(iClient);
				}
...
//		return;
	...
	CreateTimer(0.25, Timer_RemoveUberHidden, client);
}
...
stock StunRobots()
{
	for (new i = 1; i <= MaxClients; i++)
		if (IsClientInGame(i) && GetClientTeam(i) == _:TFTeam_Blue && !IsFakeClient(i))
			if(iRobotMode[i] != Robot_SentryBuster && iRobotMode[i] != Robot_Giant && iRobotMode[i] != Robot_BigNormal)// added bignormal robots to not be stunned
			{
...
public Action:OnTriggerGateTouch( iEntity, iOther )
{
	...
	if(IsValidClient(iOther) && IsClientInGame(iOther) && IsPlayerAlive(iOther))
	{
		if( GetClientTeam(iOther) != _:TFTeam_Blue || !IsGateBotPlayer[iOther] || iRobotMode[iOther] == Robot_SentryBuster || TF2_IsPlayerInCondition( iOther, TFCond_Disguised ) )// this lagged for some reason now it doesn;'t...
			return Plugin_Handled;
	}
...
public Action:NormalSoundHook( iClients[64], &iNumClients, String:strSound[PLATFORM_MAX_PATH], &iEntity, &iChannel, &Float:flVolume, &iLevel, &iPitch, &iFlags )
{
	...
	else if( StrContains( strSound, "vo/", false ) != -1 )
	{
		...
		//	ReplaceString( strSound, sizeof( strSound ), "spy_mvm", "spy", false );
...
stock PickRandomRobot( iClient, bool:bChangeClass = true )
{
	...
	{
		flLastSentryBuster = GetGameTime()+360.0;// disallow next buster for 360s
		g_CanDispatchSentryBuster = false;
...
stock PickRandomClassVariant( TFClassType:iClass = TFClass_Unknown )
{
	...
	switch( iClass )
	{
		case TFClass_Medic:		return !bGiants ? GetRandomInt(0,5) : GetRandomInt(0,6); //giant medic is re-enabled again
