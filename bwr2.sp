...
#define PLUGIN_VERSION				"BWR2 1.5.1"
...
#define SENTRYBUSTER_DAMAGE			2500
...
///SENTRY VISION//
//////////////////
new bool:WasCarried[2069];
new ObjGlow[2069];
new Handle:ObjTimer[2069];
...
public OnPluginStart()
{
	...
	sm_tf2bwr_logs = CreateConVar( "sm_tf2bwr_logs", "0", "", FCVAR_PLUGIN, true, 0.0, true, 1.0 ); //"1" disabled logs cos reasons
	...
	sm_tf2bwr_respawn_blue = CreateConVar( "sm_tf2bwr_respawn_blue", "3", "Respawn fix for BLU team. Set -1 to disable it.", FCVAR_PLUGIN, true, -1.0 );
	...
	sm_tf2bwr_engineers = CreateConVar( "sm_tf2bwr_engineers", "2", "Allow/disallow engineers", FCVAR_PLUGIN, true, -1.0, true, 10.0 ); //changed to 2 engineers
...
//	AddTempEntHook( "TFParticleEffect", TEHook_Particle );
	...
	HookEvent( "player_upgradedobject", OnBuildUp);
	HookEvent( "player_carryobject", OnBuildCarry);
...
{
	PrintToChatAll("You're playing BWR2 Version %s", PLUGIN_VERSION);
...
//		}
	...
	if( iClass == TFClass_Spy && GameRules_GetRoundState() == RoundState_RoundRunning )//spytele
	{
		CreateTimer( 3.0, Timer_TeleportSpy, iClient);
...
//	bInRespawn[iClient] = true;
	...
	//	IsGateBotPlayer[iClient] = false;
	//}
	if(IsMvM() && !IsValidRobot(iClient) && !GateStunEnabled)
	{
		SpawnRobot(iClient);
	}
	if(!IsMvM() || !IsValidClient(iClient) || IsFakeClient(iClient))
	...
	new TFClassType:class = TF2_GetPlayerClass(iClient);
	if(iRobotMode[iClient] == Robot_Giant && class == TFClass_Pyro )
	{
		TF2Attrib_SetByName(iClient, "deflection size multiplier", 1.1);
...
public Action:Timer_TeleportSpy( Handle:Timer, any:client )//spytele
{
	...
	if( iClass == TFClass_Spy && TF2_IsPlayerInCondition( client, TFCond_Cloaked ) )
	{
		...
		{
			TeleportEntity(client, SpyHintPos, NULL_VECTOR, NULL_VECTOR);
			TF2_AddCondition(client, TFCond_UberchargedHidden, 0.55);
			TF2_AddCondition(client, TFCond_UberchargeFading, 0.55);
...
FindNearestSpyHint()
{
	...
	new Float:aux_dist;
	new i5 = -1;
	while((i5 = FindEntityByClassname(i5, "func_tfbot_hint")) != -1)
	{
		...
		if (IsClientInGame(i) && GetClientTeam(i) == _:TFTeam_Red)
		{
			...
			if(aux_dist < found_dist && aux_dist > 600) // to not spawn in the line of fire
			{
				found = i5;
				found_dist = aux_dist;
			}
...
stock FindRandomSpawnSpyPoint( iType )//spytele
{
	...
	new iEnt = -1;
	while( ( iEnt = FindEntityByClassname( iEnt, "func_tfbot_hint") ) != -1 )
	{
...
public TF2Items_OnGiveNamedItem_Post(client, String:strClassName[], iItemDefinitionIndex, itemLevel, itemQuality, entityIndex)
{
	if( IsMvM() )
	{
		SetEntPropEnt(entityIndex, Prop_Send, "m_hOwnerEntity", client);
		SetEntProp(entityIndex, Prop_Send, "m_bValidatedAttachedEntity", 1);//item visiblity
	}
}
	...
	switch( iClass )
	{
		case TFClass_Scout:
		{
			...
			if( iRobotVariant[iClient] == 1 ) //bonk scout
				SpawnWeapon( iClient, "tf_weapon_lunchbox_drink", 46, 100, 5, weaponAttribs, false );
			if( iRobotVariant[iClient] == 0 || iRobotVariant[iClient] == 1 || iRobotVariant[iClient] == 5 )
			{
				Format(weaponAttribs, sizeof(weaponAttribs), "704 ; 0");
				if( iRobotVariant[iClient] == 5 ) // Giant scout given scattergun
				{
			...
			{
				Format(weaponAttribs, sizeof(weaponAttribs), "182 ; 5 ; 278 ; 0.3");
				SpawnWeapon( iClient, "tf_weapon_bat_giftwrap", 648, 100, 5, weaponAttribs, false );
			}
		}
		case TFClass_Soldier:
		{
			if(iRobotVariant[iClient] == 6 || iRobotVariant[iClient] == 7 || iRobotVariant[iClient] == 8 || iRobotVariant[iClient] == 9) //giant soldiers with blackbox one
			{
				if(GetRandomInt(0,3) > 2 && GameRules_GetRoundState() == RoundState_RoundRunning)//25% to get anybanner
				{
					new BannerSwitch = -1;
					if(Is666Mode)
						BannerSwitch = GetRandomInt(1,2);
					else
						BannerSwitch = GetRandomInt(1,3);
					switch(BannerSwitch)
					{
						case 1:
						{
							Format(weaponAttribs, sizeof(weaponAttribs), "116 ; 3 ; 319 ; 9");
							SpawnWeapon( iClient, "tf_weapon_buff_item", 354, 100, 5, weaponAttribs, false );
							PrintToChat( iClient, "You have been spawned with Concheror");
						}
						case 2:
						{
							Format(weaponAttribs, sizeof(weaponAttribs), "116 ; 2 ; 319 ; 9");
							SpawnWeapon( iClient, "tf_weapon_buff_item", 226, 100, 5, weaponAttribs, false );
							PrintToChat( iClient, "You have been spawned with Battalion Backup");
						}
						case 3:
						{
							Format(weaponAttribs, sizeof(weaponAttribs), "319 ; 9");
							SpawnWeapon( iClient, "tf_weapon_buff_item", 129, 100, 5, weaponAttribs, false );
							PrintToChat( iClient, "You have been spawned with Buff Banner");
						}
					}
				}
			}
			...
			{
				Format(weaponAttribs, sizeof(weaponAttribs), "2 ; 0.45 ; 318 ; 0.9 ; 16 ; 60 ; 411 ; 2 ; 99 ; 1.25 ; 6 ; 0.001");
			...
			{
				Format(weaponAttribs, sizeof(weaponAttribs), "2 ; 0.45 ; 318 ; 1.6 ; 16 ; 334 ; 411 ; 4 ; 99 ; 1.25 ; 6 ; 0.001 ; 107 ; 0.5 ; 252 ; 0.4 ; 329 ; 0.4 ; 103 ; 0.9 ; 140 ; 5800 ; 330 ; 3"); // 334 hp per hit  6200 max hp
				SpawnWeapon( iClient, "tf_weapon_rocketlauncher", 228, 100, 5, weaponAttribs, false );
			}
			...
			{
				if( iRobotVariant[iClient] == 1 ) // normal buff soldier replaced by rapid strike
				{
					Format(weaponAttribs, sizeof(weaponAttribs), "116 ; 3 ; 319 ; 5");
					SpawnWeapon( iClient, "tf_weapon_buff_item", 354, 100, 5, weaponAttribs, false );
					
					Format(weaponAttribs, sizeof(weaponAttribs), "6 ; 0.33 ; 522 ; 1 ; 114 ; 1 ; 240 ; 0.50"); //Rapid strike
					SpawnWeapon( iClient, "tf_weapon_rocketlauncher_airstrike", 1104, 100, 5, weaponAttribs, false );
				}
				else if( iRobotVariant[iClient] == 2 ) //buff banner ext
				{
					Format(weaponAttribs, sizeof(weaponAttribs), "319 ; 9");
					SpawnWeapon( iClient, "tf_weapon_buff_item", 129, 100, 5, weaponAttribs, false );
				}
				else if( iRobotVariant[iClient] == 3 ) //conch
				{
					Format(weaponAttribs, sizeof(weaponAttribs), "116 ; 3 ; 319 ; 9");
					SpawnWeapon( iClient, "tf_weapon_buff_item", 354, 100, 5, weaponAttribs, false );
				}
				else if( iRobotVariant[iClient] == 4 ) // backup
				{
					Format(weaponAttribs, sizeof(weaponAttribs), "116 ; 2 ; 319 ; 9");
					SpawnWeapon( iClient, "tf_weapon_buff_item", 226, 100, 5, weaponAttribs, false );
				}
			...
			SpawnWeapon( iClient, "tf_weapon_shovel", 6, 100, 5, weaponAttribs, false );
		...
		case TFClass_Pyro:
		{
			...
			{
				Format(weaponAttribs, sizeof(weaponAttribs), "2 ; 1.0 ; 6 ; 0.75 ; 318 ; 1.25 ; 103 ; 0.35");
		...
		case TFClass_DemoMan:
		{
			...
			{
				Format(weaponAttribs, sizeof(weaponAttribs), "26 ; 2325 ; 107 ; 2 ; 252 ; 0.5 ; 329 ; 0.5 ; 330 ; 7 ; 402 ; 1 ; 137 ; 75 ; 396 ; 0.5");
			...
			else
			{
				...
				else if( iRobotVariant[iClient] == 1 || iRobotVariant[iClient] == 4 ) //demoknights
				{
					Format(weaponAttribs, sizeof(weaponAttribs), "704 ; 0");
		...
		case TFClass_Heavy:
		{
			...
			else if( iRobotVariant[iClient] == 1 )
			{
				...
				Format(weaponAttribs, sizeof(weaponAttribs), "107 ; 1.3 ; 2 ; 0.2 ; 125 ; -240");
			...
			{
				Format(weaponAttribs, sizeof(weaponAttribs), "704 ; 0");
				SpawnWeapon( iClient, "tf_weapon_fists", 239, 100, 5, weaponAttribs, false );
			}
		...
		case TFClass_Medic:
		{
			...
			{
				Format(weaponAttribs, sizeof(weaponAttribs), "479 ; 0");
		...
		case TFClass_Sniper:
		{
			...
			if( iRobotVariant[iClient] == 0 || iRobotVariant[iClient] == 1 )
				SpawnWeapon( iClient, "tf_weapon_sniperrifle", 14, 100, 5, weaponAttribs, false );
			else if( iRobotVariant[iClient] == 2 )
				SpawnWeapon( iClient, "tf_weapon_sniperrifle", 230, 100, 5, weaponAttribs, false );
			else if( iRobotVariant[iClient] == 4 )
				SpawnWeapon( iClient, "tf_weapon_compound_bow", 56, 100, 5, weaponAttribs, false );
			
			if( iRobotVariant[iClient] == 1 )
				SpawnWeapon( iClient, "tf_wearable", 57, 100, 5, weaponAttribs, true );
			else if( iRobotVariant[iClient] == 3 )
				SpawnWeapon( iClient, "tf_weapon_jar", 58, 100, 5, weaponAttribs, false );
//			else if( iRobotVariant[iClient] == 0 || iRobotVariant[iClient] == 2 || iRobotVariant[iClient] == 4 )
//				SpawnWeapon( iClient, "tf_weapon_smg", 16, 100, 5, weaponAttribs, false );
			SpawnWeapon( iClient, "tf_weapon_club", 3, 100, 5, weaponAttribs, false );
			if(IsGateBotPlayer[iClient])
			{
				Format(weaponAttribs, sizeof(weaponAttribs), "134 ; 64");
		...
		case TFClass_Engineer:
		{
			if( iRobotVariant[iClient] == 0 )
			{
				...
				//SpawnWeapon( iClient, "tf_weapon_pistol", 22, 100, 5, weaponAttribs, false );
			...
			/*else if( iRobotVariant[iClient] == 2 ) //Future engineer
			{
				Format(weaponAttribs, sizeof(weaponAttribs), "140 ; 175 ; 353 ; 1 ; 94 ; 5 ; 345 ; 2");
				SpawnWeapon( iClient, "tf_weapon_wrench", 589, 100, 5, weaponAttribs, false ); //eureka effect
				Format(weaponAttribs, sizeof(weaponAttribs), "208 ; 1 ; 97 ; 0.5 ; 6 ; 0.8 ; 407 ; 1");
				SpawnWeapon( iClient, "tf_weapon_drg_pomson", 588, 100, 5, weaponAttribs, false ); //pomson
				Format(weaponAttribs, sizeof(weaponAttribs), "704 ; 0");
				//SpawnWeapon( iClient, "tf_weapon_pistol", 22, 100, 5, weaponAttribs, false );
				SpawnWeapon( iClient, "tf_weapon_pda_engineer_build", 25, 100, 5, weaponAttribs, false );
				SpawnWeapon( iClient, "tf_weapon_pda_engineer_destroy", 26, 100, 5, weaponAttribs, false );
				...
				Format(weaponAttribs, sizeof(weaponAttribs), "295 ; 1");
				SpawnWeapon( iClient, "tf_wearable", 519, 100, 5, weaponAttribs, true );
			}*/
			if(IsGateBotPlayer[iClient])
			{
				Format(weaponAttribs, sizeof(weaponAttribs), "134 ; 64");
...
public Action:Timer_RemoveCond(Handle:Timer, client)
{
...
//		return;
	...
	if(IsMannhattan && GameRules_GetRoundState() == RoundState_RoundRunning && class != TFClass_Sniper && class != TFClass_Spy && class != TFClass_Engineer && class != TFClass_Medic && ((LastGateCapture + 19.9) < GetEngineTime()) && !IsGateBotPlayer[client] && nGateCapture != 2 && !IsFakeClient(client))
	{
		new iflagr = -1;
		if(CanTeleportBomb1)
		{
...
stock StunRobots()
{
	for (new i = 1; i <= MaxClients; i++)
		if (IsClientInGame(i) && GetClientTeam(i) == _:TFTeam_Blue && !IsFakeClient(i))
			if(iRobotMode[i] != Robot_SentryBuster && iRobotMode[i] != Robot_Giant)
			{
...
public Action:OnTakeDamage( iVictim, &iAttacker, &iInflictor, &Float:flDamage, &iDamageBits, &iWeapon, Float:flDamageForce[3], Float:flDamagePosition[3], iDamageCustom )
{
	...
	if( (iDamageBits != TF_DMG_AFTERBURN) && !IsAttackerSentry(iInflictor))
	{
		...
		if( GetClientTeam(iVictim) == _:TFTeam_Red && TF2_IsPlayerInCondition( iAttacker, TFCond_UberchargedHidden ) )
		{
			flDamage = 0.0;
			return Plugin_Changed;
		}
...
//		return Plugin_Changed;
//	}
	if( GetClientTeam(iAttacker) == _:TFTeam_Blue && TF2_IsPlayerInCondition( iAttacker, TFCond_UberchargedHidden ) && !IsAttackerSentry(iInflictor) ) // && IsValidEntity( iBuilding ) )
	{
		flDamage = 0.0;
		return Plugin_Changed;
	}
...
stock PickRandomRobot( iClient, bool:bChangeClass = true )
{
	...
	if(g_CanDispatchSentryBuster==true && iRobotMode[iClient] != Robot_SentryBuster && GetRandomInt(0,9) > 8 && !IsFakeClient(iClient)) //10%
	{
	...
	switch( iClass )
	{
		...
		case TFClass_Engineer:	
		{
			...
			if(GameRules_GetRoundState() == RoundState_RoundRunning && EngiNest != -1)
				return GetRandomInt(0,1); //removed future engineer
			else
				return 0;
		}
	}
...
//					if( iVariant >= 6 && iVariant <= 8 )
//						iNewEffect2 = Effect_FullCharge;
					else if( iVariant == 7 )
	...
	switch( iClass )
	{
		case TFClass_Scout:
		{
			switch( iVariant )
			{
				...
				case 2: strcopy( strBuffer, iBufferSize, "Knockback Scout" ); //old unused fish scout replaced by Knockback scout
				case 3: strcopy( strBuffer, iBufferSize, "Minor League Scout" );
				case 4: strcopy( strBuffer, iBufferSize, "Wrap Assasin" );
		...
		case TFClass_Soldier:
		{
			switch( iVariant )
			{
				...
				case 1: strcopy( strBuffer, iBufferSize, "Rapid Strike Soldier" );
		...
		case TFClass_DemoMan:
		{
			switch( iVariant )
			{
				...
				case 3: strcopy( strBuffer, iBufferSize, "Giant Blast Demoman" );
		...
		case TFClass_Engineer:
		{
			switch( iVariant )
			{
				...
				case 2: strcopy( strBuffer, iBufferSize, "Future Engineer" );
			}
		}
	}
...
{
		if(!( GetEntityFlags(Deployer) & FL_ONGROUND )) //bugfix for deploy when touching deploy area while in air would make deploy execute
		{
			if (g_hDeployTimer != INVALID_HANDLE)
			{
				CloseHandle(g_hDeployTimer);
...
//		TF2_AddCondition(Deployer, TFCond_UberchargedHidden, 1.0);
		...
		FixSounds(Deployer);
//		CreateTimer(0.10, Timer_CaptureIntel);
		CaptureIntel();
...
stock AttachParticle(entity, String:particleType[], Float:offset[]={0.0,0.0,0.0})
{
	new particle=CreateEntityByName("info_particle_system");
...
//}
	...
	if( GameRules_GetRoundState() == RoundState_RoundRunning && bEnableBuster && !g_CanDispatchSentryBuster ) // && iRobotMode[iClient] != Robot_SentryBuster && bEnableBuster)  // && GetRandomInt(0,9) == 0 && CheckCommandAccess( iClient, "tf2bwr_sentrybuster", 0, true ) )
	{
		...
		while( ( iSentry = FindEntityByClassname( iSentry, "obj_sentrygun" ) ) != -1 )
		{
			...
			if( GetEntProp( iSentry, Prop_Send, "m_iTeamNum" ) == _:TFTeam_Red && IsThereAnyRedEngineer()) // && SentryBusterQueNotClosed==true)
			{
	...
	if( GetClientTeam(client) == _:TFTeam_Blue && !IsFakeClient( client ) && !bInRespawn[client] && GameRules_GetRoundState() == RoundState_RoundRunning && iAttacker > 0 && iAttacker != client ) //&& !TF2_IsPlayerInCondition( client, TFCond_Cloaked ) )//GameRules_GetRoundState() != RoundState_BetweenRounds
	{
...
//		SetEntityMoveType(MoneypackL, MOVETYPE_FLYGRAVITY);
		DispatchSpawn(MoneypackL);
//		ActivateEntity(MoneypackL);
//		PrintToChatAll("Check succes");
		...
		//CreateTimer( 15.0, Timer_CashTele, MoneypackL );
		//GroundEntity(MoneypackL);
		//CreateTimer( 28.0, Timer_CashKill, MoneypackL );
		
		//if(!( GetEntityFlags(client) & FL_ONGROUND ) )
		//{
		CreateTimer( 0.1, Timer_CashTele, MoneypackL );
		//PrintToChatAll("Teleported cash trigger!");
		//}
	}
	return Plugin_Continue;
}
...
public TF2_OnConditionAdded(client, TFCond:condition) 
{
	...
	new TFClassType:class = TF2_GetPlayerClass(client);
	if(condition==TFCond_Taunting && GetClientTeam(client) == _:TFTeam_Blue && class == TFClass_Engineer)//congafix
	{
		GetClientUserId( client );
...
//		else Causes to glitch mann works
...
public Action:Timer_SetInstantLevel3( Handle:hTimer, any:iEntity )
{
	new String:sEnt[255];
	Entity_GetClassName(iEntity,sEnt,sizeof(sEnt));
	if (!StrEqual(sEnt, "obj_sentrygun"))
		return Plugin_Stop;
//	if( GetEntProp( iEntity, Prop_Send, "m_iTeamNum" ) == _:TFTeam_Red )
//		DispatchKeyValue( iEntity, "targetname", "bwrredsentry"); 
...
public Action:Timer_DefenseBuff( Handle:hTimer, any:client)
{
	...
	for(new i=1; i<=MaxClients; i++)
	{
		if(IsValidClient(i) && GetClientTeam(i) == _:TFTeam_Blue) //revisited to make code less resource hungry
		{
			new Float:flPos2[3];
			GetClientAbsOrigin(i, flPos2);
			...
			if(!TF2_IsPlayerInCondition( i, TFCond_DefenseBuffNoCritBlock ))
			{
				if(flDistance < 395.0) // && GetClientTeam(i) == _:TFTeam_Blue)
					TF2_AddCondition(i, TFCond_DefenseBuffNoCritBlock, 3.0, client);
			}
		}
	}
	return Plugin_Continue;
...
stock CheckTeleportClamping(Teleporter)
{
	// roof checks [5]
	// roof trace straight
	new Float:flPos[3], Float:flAng[3];
...
stock DisplayClamping(Teleporter, Float:endPos[3])
{
	...
	PrintCenterText(TeleOwner,"Too small space for teleporter!");
...
//////			GLOW		///////
//////////////////////////////////
public Action:OnBuildObject( Handle:hEvent, const String:strEventName[], bool:bDontBroadcast )
{
	new obj = GetEventInt(hEvent,"index");
	if( GetEntProp( obj, Prop_Send, "m_iTeamNum" ) == _:TFTeam_Red )
	{
		if(2069 > obj > -1)
		{
			CreateTimer(0.4,Glow,obj);
			if(GameRules_GetRoundState() == RoundState_RoundRunning && !WasCarried[obj])
				ObjTimer[obj] = CreateTimer(0.1,NewGlow,obj,TIMER_REPEAT);
		}
	}
}
public Action:OnBuildUp( Handle:hEvent, const String:strEventName[], bool:bDontBroadcast )
{
	new obj = GetEventInt(hEvent,"index");
	if(2069 > obj > -1)
	{
		CreateTimer(2.0,Glow,obj);
		if(IsValidEntity(ObjGlow[obj]))
		{
...
public Action:OnBuildCarry( Handle:hEvent, const String:strEventName[], bool:bDontBroadcast )
{
	new obj = GetEventInt(hEvent,"index");
	if(2069 > obj > -1)
	{
...
public Action:Glow(Handle:timer,any:obj)
{
	GlowBuilding(obj);
}
public Action:NewGlow(Handle:timer,any:obj)
{
	if(!IsValidEntity(obj)) return Plugin_Stop;
	decl String:sBuffer[PLATFORM_MAX_PATH];
	GetEntPropString(obj, Prop_Data, "m_ModelName", sBuffer, sizeof(sBuffer));
	if(StrContains(sBuffer, "sentry") != -1)
	{
		if(StrContains(sBuffer, "heavy") == -1)
		{
...
//			ObjTimer[obj] = INVALID_HANDLE;
	...
	{
		if(StrContains(sBuffer, "light") != -1)
		{
			CreateTimer(0.1,Glow,obj);
			if(IsValidEntity(ObjGlow[obj]))
				RemoveEdict(ObjGlow[obj]);	
...
GlowBuilding(iEnt)
{
	if(ObjGlow[iEnt] != -1 || !GetConVarBool(cvarOutlineEnable) || !IsValidEntity(iEnt))
		return;
